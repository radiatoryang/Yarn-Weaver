...
//namespace Yarn
//{
	public class YarnWeaverFileFormatConverter
	{
...
//			YarnSpinnerConsole.Error(string.Format("You must specify a destination format. Run '{0} help convert' to learn more.", processName));
		...
		public static int ConvertToJSON(ConvertFormatOptions options)
		{
			foreach (var file in options.files)
			{
				...
				if (YarnWeaverLoader.GetFormatFromFileName(file) == NodeFormat.JSON)
				{
					Debug.LogWarning(string.Format("Not converting file {0}, because its name implies it's already in JSON format", file));
					continue;
				}

				ConvertNodesInFile(options, file, "json", (IEnumerable<YarnWeaverLoader.NodeInfo> nodes) => JsonConvert.SerializeObject(nodes, Formatting.Indented));
		...
		public static int ConvertToYarn(ConvertFormatOptions options)
		{
			foreach (var file in options.files)
			{
				if (YarnWeaverLoader.GetFormatFromFileName(file) == NodeFormat.Text)
				{
		...
		public static string ConvertNodes(IEnumerable<YarnWeaverLoader.NodeInfo> nodes, NodeFormat format) {
			switch (format)
			{
		...
		public static string ConvertNodesToYarnText(IEnumerable<YarnWeaverLoader.NodeInfo> nodes)
		{
			var sb = new System.Text.StringBuilder();

			var properties = typeof(YarnWeaverLoader.NodeInfo).GetProperties();
			...
			foreach (var node in nodes) {
				...
				foreach (var property in properties) {
					...
					else if (propertyType.IsAssignableFrom(typeof(YarnWeaverLoader.NodeInfo.Position)))
					{
						var position = (YarnWeaverLoader.NodeInfo.Position)property.GetValue(node, null);
		...
		public delegate string ConvertNodesToText(IEnumerable<YarnWeaverLoader.NodeInfo> nodes);
		...
		public static void ConvertNodesInFile(ConvertFormatOptions options, string file, string fileExtension, ConvertNodesToText convert)
		{
			...
			IEnumerable<YarnWeaverLoader.NodeInfo> nodes;
			try {
				nodes = YarnWeaverLoader.GetNodesFromText(text, YarnWeaverLoader.GetFormatFromFileName(file));
